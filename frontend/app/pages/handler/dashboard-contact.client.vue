 ...
 * ТИП: СОЗДАНИЕ
 * - Новый placement-handler для вкладки дашборда в карточке контакта.
 * - Подключён фронтенд-расчёт KPI через useDealStats.
 * УДАЛЕНИЕ:
...
const { $logger, initApp, b24Helper, destroyB24Helper, processErrorGlobal } = useAppInit('dashboard_contact')
 ...
 * Приоритет:
 * 1) placement.options.ID (stable for placements)
...
function resolveContactId(frame: B24Frame): number | string {
...
onMounted(async () => {
  try {
    ...
    entityId.value = resolveContactId($b24)
    $logger.info(`Contact ID: ${entityId.value}`, {
      placementOptions: $b24.placement?.options,
      context: $b24.getContext?.()
    })
    if (entityId.value === 'unknown') {
      $logger.warn('Contact ID was not found in placement options or context')
    }
     ...
     */
    // Статистику сделок загружаем с фронтенда через REST SDK для текущего контакта.
    dealStats = useDealStats($b24, entityId.value, 'contact')
  ...
  <div class="p-4">
    <div v-if="isInit && dealStats" class="space-y-4">
      ...
      <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Контакт ID: {{ entityId }}</p>
      </div>
